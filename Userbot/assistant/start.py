...
# Tambahan untuk card mode
from PIL import Image, ImageDraw, ImageFont, ImageOps

FONT_PATH = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
#CARD_BG_PATH = "Userbot/picture/Artboard 1.png"

CARD_BG_DIR = "Userbot/picture"
DEFAULT_CARD_BG = "Artboard 1.png"  # Default fallback
...
def get_artboard_list():
    if not os.path.exists(CARD_BG_DIR):
        return []
    return [
        f for f in os.listdir(CARD_BG_DIR)
        if ("Artboard" in f and f.lower().endswith(('.png', '.jpg', '.jpeg')))
    ]

def get_selected_cardbg():
    # Ambil nama file gambar dari DB
    filename = dB.get_var(bot_id, "CARDBG_PIC")
    if filename and filename in get_artboard_list():
        return os.path.join(CARD_BG_DIR, filename)
    # fallback
    return os.path.join(CARD_BG_DIR, DEFAULT_CARD_BG)

def download_card_bg():
    bg_path = get_selected_cardbg()
    if os.path.exists(bg_path):
        return Image.open(bg_path).convert("RGBA").resize((800, 400))
    else:
        print(f"Error: {bg_path} tidak ditemukan.")
...
@bot.on_message(filters.command("listartboard", prefixes=[".", "/"]) & filters.user(owner_id))
async def listartboard_command(client, message: Message):
    files = get_artboard_list()
    if not files:
        return await message.reply("Tidak ada file Artboard yang ditemukan di folder Userbot/picture.")
    text = "<b>Daftar Artboard yang tersedia:</b>\n"
    for f in files:
        selected = ""
        if f == dB.get_var(bot_id, "CARDBG_PIC"):
            selected = " <b>(aktif)</b>"
        text += f"• <code>{f}</code>{selected}\n"
    await message.reply(text)

@bot.on_message(filters.command("setartboard", prefixes=[".", "/"]) & filters.user(owner_id))
async def setartboard_command(client, message: Message):
    if len(message.command) < 2:
        return await message.reply("Format: <code>.setartboard [namapicture]</code>\nGunakan .listartboard untuk melihat daftar.")

    filename = " ".join(message.command[1:])  # support nama file dengan spasi
    files = get_artboard_list()
    if filename not in files:
        return await message.reply(
            f"File <code>{filename}</code> tidak ditemukan di folder Userbot/picture.\n"
            "Gunakan <code>.listartboard</code> untuk melihat daftar yang tersedia."
        )
    dB.set_var(bot_id, "CARDBG_PIC", filename)
    return await message.reply(
        f"✅ Artboard untuk kartu profil diatur ke <code>{filename}</code>.\n"
        "Sekarang gunakan <code>/setpicstart card</code> lalu cek /start di userbot."
    )
...
async def setpicstart_command(client, message: Message):
    ...
    if len(message.command) > 1 and message.command[1].lower() == "card":
        ...
        return await message.reply("✅ Mode start card berhasil diaktifkan!\n"
                                  "Background card: <code>{}</code>".format(dB.get_var(bot_id, "CARDBG_PIC") or DEFAULT_CARD_BG))
