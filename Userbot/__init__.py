...
#@moire_mor
    async def broadcaster_cluster(
        ...
        try:
            await client.send_message(
                message_ids["chat_id"],
                f"<blockquote><b><emoji id=4915853119839011973>⚠️</emoji> Terdeteksi bot melakukan restart pada cluster {group_name}.\n<emoji id=5350490470025816976>🚀</emoji> Auto Start Broadcast.</b>\n └<i> Menunggu interval...</i></blockquote>",
                reply_to_message_id=message_ids["message_id"]
            )
        except Exception as e:
            print(f"<blockquote><i><emoji id=4918014360267260850>⛔️</emoji> Gagal mengirim pesan awal: {e}</i></blockquote>")
        ...
        putaran = dB.get_var(user_id, f'INTERVAL_COUNT_{group_name}') or 0
        while putaran < max_loops:
            if (
                ...
                try:
                    await client.send_message(
                        message_ids["chat_id"],
                        f"<blockquote><i><emoji id=5350740982583301822>🗑</emoji> Menghapus data message_ids {group_name} dari database</blockquote></i>",
                        reply_to_message_id=message_ids["message_id"]
                    )
                finally:
                    dB.set_var(user_id, f"MESSAGE_IDS_{group_name}", {})
                    dB.set_var(user_id, f"INTERVAL_COUNT_{group_name}", 0)
                break
            ...
            if auto_off_time and datetime.now() > auto_off_time:
                ...
                try:
                    await client.send_message(
                        message_ids["chat_id"],
                        f"<blockquote><b><emoji id=5350423812133381005>🕔</emoji> Waktu yang ditentukan telah dilampaui</b>.\n  └<i> Menghentikan Broadcast cluster {group_name}.</blockquote></i>",
                        reply_to_message_id=message_ids["message_id"]
                    )
                finally:
                    dB.set_var(user_id, f"MESSAGE_IDS_{group_name}", {})
            ...
            report = f"""<blockquote><b><emoji id=5350404703823883106>⭐️</emoji> Hasil Broadcast <b>{group_name}</b></b></blockquote>
<blockquote>╭─❖ Ringkasan
├ <emoji id=5317059204802952215>🖱️</emoji> Status: Selesai
├ <emoji id=5317059204802952215>🖱️</emoji> Berhasil : {success_count} grup
├ <emoji id=5317059204802952215>🖱️</emoji> Gagal : {fail_count} grup
...
"""
    ...
    async def loop_restart_cluster(self):
        for client in self._ubot:
            user_id = client.me.id
            ...
            for group_name in GROUP_CLUSTERS:
                message_ids = dB.get_var(user_id, f"MESSAGE_IDS_{group_name}") or {}
                if not message_ids:
                    continue
                if not dB.get_var(user_id, f"BC_RUNNING_{group_name}"):
                    continue

                self.status_background[user_id] = True

                try:
                    chat_ids = dB.get_list_from_var(user_id, f"BC_TARGET_GROUPS_{group_name}") or []
                    if not chat_ids:
                        print(f"Tidak ada grup target (BC_TARGET_GROUPS_{group_name}) untuk broadcast user {user_id}")
                        continue
                except Exception as e:
                    print(f"Error mengambil daftar grup target broadcast user {user_id} cluster {group_name}: {e}")
                    continue
                ...
                asyncio.create_task(
                    self.broadcaster_cluster(
                        client, user_id, group_name, message_ids, chat_ids, blacklist,
                        delay_per_chat, delay_between_intervals, max_loops, auto_off_time
                    )
                )
